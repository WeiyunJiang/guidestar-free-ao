# This script trains a U-Net model to predict the psf from the blurred image.
# In this script, we create the blurred, psf phase pair on fly, and train the model on the fly
# The input to the model is patchified blurred image and the output is the cropped psf


import argparse
import os
import torch
import torch.nn as nn
import time
import numpy as np
import random
import torch.nn.functional as F

from network import Unet, BlurKernelUNet64x64
from dataio import Places2, PSF_Phase_Wrapper, generate_uniform_random_vector, generate_amp_psf_and_phase
from torch.utils.data import DataLoader
from utils import cond_mkdir, log_psf_and_blurred_to_tensorboard, resume_training, create_central_mask, masked_mse_loss, torch_convolve2d, central_crop, patchify_blurred_img, add_gaussian_noise
from torch.utils.tensorboard import SummaryWriter
from tqdm import tqdm
# os.environ['OMP_NUM_THREADS'] = '1'
# os.environ['MKL_NUM_THREADS'] = '1'
# os.environ['OPENBLAS_NUM_THREADS'] = '1'
# torch.set_num_threads(4)

parser = argparse.ArgumentParser()
parser.add_argument('--gpu', type=str, default='5')

parser.add_argument('--train_size', type=int, default=100000)
parser.add_argument('--valid_size', type=int, default=2000)

parser.add_argument('--psf_dir_train', nargs='+', type=str, 
                    default=['/home/wj22/pupil/data/triangle_1m_psf_1.5_new/train/psf', '/home/wj22/pupil/data/triangle_1m_psf_1.3_new/train/psf', '/home/wj22/pupil/data/triangle_1m_psf_1.1_new/train/psf', '/home/wj22/pupil/data/triangle_1m_psf_0.9_new/train/psf', 
                    '/home/wj22/pupil/data/triangle_2m_psf_0.7_new/train/psf', '/home/wj22/pupil/data/1m_psf_0.5_new/train/psf', '/home/wj22/pupil/data/500k_psf_0.3_new/train/psf',
                    '/home/wj22/pupil/data/triangle_200k_psf_0.1_new/train/psf', '/home/wj22/pupil/data/triangle_100k_psf_0.01_new/train/psf'],
                    help="Paths to training PSF directories.")
parser.add_argument('--phase_dir_train', nargs='+', type=str, 
                    default=['/home/wj22/pupil/data/triangle_1m_psf_1.5_new/train/phase', '/home/wj22/pupil/data/triangle_1m_psf_1.3_new/train/phase', '/home/wj22/pupil/data/triangle_1m_psf_1.1_new/train/phase', '/home/wj22/pupil/data/triangle_1m_psf_0.9_new/train/phase',
                    '/home/wj22/pupil/data/triangle_2m_psf_0.7_new/train/phase','/home/wj22/pupil/data/1m_psf_0.5_new/train/phase', '/home/wj22/pupil/data/500k_psf_0.3_new/train/phase',
                    '/home/wj22/pupil/data/triangle_200k_psf_0.1_new/train/phase', '/home/wj22/pupil/data/triangle_100k_psf_0.01_new/train/phase'], 
                    help="Paths to training phase directories.")
parser.add_argument('--psf_dir_val', nargs='+', type=str, 
                    default=['/home/wj22/pupil/data/triangle_1m_psf_1.5_new/val/psf'],
                    help="Paths to validation PSF directories.")
parser.add_argument('--phase_dir_val', nargs='+', type=str, 
                    default=['/home/wj22/pupil/data/triangle_1m_psf_1.5_new/val/phase'],
                    help="Paths to validation phase directories.")



parser.add_argument('--batch_size', type=int, default=256)
parser.add_argument('--epochs', type=int, default=5000000)
parser.add_argument('--lr', type=float, default=2e-4)  
parser.add_argument('--loss_type', type=str, default='l2')
parser.add_argument('--model_type', type=str, default='unet') #BlurKernelUNet64x64
parser.add_argument('--patch_size', type=int, default=64)
# parser.add_argument('--mask_type', type=str, default='triangle')
parser.add_argument('--patches_shuffle', type=bool, default=False)
parser.add_argument('--gaussian_noise', type=bool, default=False)
parser.add_argument('--name', type=str, default='patched_ker_est_unet_lr_2e-4_places2_100k_img_7.8m_psf_1.5_num_down_5_max_channels_4096_nf0_128_bz_256_triangle')
args = parser.parse_args()

print(f'patches_shuffle: {args.patches_shuffle}')

resume = False
os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
device = torch.device(f'cuda' if torch.cuda.is_available() else 'cpu')
print('Using device:', device)
print(torch.cuda.get_device_name(0))

seed = 42

random.seed(seed)          # Python's random module
np.random.seed(seed)       # NumPy
torch.manual_seed(seed)    # PyTorch

# If using CUDA:
if torch.cuda.is_available():
    torch.cuda.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
if args.model_type == "unet":
    model = Unet(in_channels=16,out_channels=1,nf0=128,num_down=5,max_channels=4096,use_dropout=False,outermost_linear=True).to(device)
elif args.model_type == "BlurKernelUNet64x64":
    model = BlurKernelUNet64x64().to(device)
else:
    raise ValueError("Model type must be 'unet' or 'BlurKernelUNet64x64'")

model.train()
model = model.to(device)
# Create the dataset for training

categories = [
    "apartment_building-outdoor",
    "cottage",
    "inn-outdoor",
    "mansion",
    "manufactured_home",
    "oast_house",
    "house",
    "village",
    "tree_house",
    "cabin-outdoor",
    "chalet",
    "home_office",
    "home_theater",
    "living_room",
    "porch",
    "residential_neighborhood",
    "shed",
    "garage-indoor",
    "garage-outdoor",
    "courtyard"
]
place2_dataset_train = Places2(categories=categories, 
                               split='train', 
                               resolution=(256, 256), 
                               downsampled=False, 
                               num_images=args.train_size, 
                               cache='none', #'in_memory',
                               psf_dir=args.psf_dir_train,
                               psf_cache='none', #'in_memory',
                               phase_dir=args.phase_dir_train,
                               phase_cache='none') #'in_memory')
# psf_phase_dataset_train = PSF_Phase_Wrapper(dataset=place2_dataset_train, mask_type=args.mask_type, device='cpu')
dataloader_train = torch.utils.data.DataLoader(place2_dataset_train, 
                                               batch_size=args.batch_size, 
                                               shuffle=True, 
                                               pin_memory=True, 
                                               num_workers=8, 
                                               persistent_workers=True)
# Create the dataset for validation
### Use train for special toy example
place2_dataset_val = Places2(categories=categories, 
                             split='val', 
                             resolution=(256, 256), 
                             downsampled=False, 
                             num_images=args.valid_size, 
                             cache='none',#'in_memory',
                             psf_dir=args.psf_dir_val,
                             psf_cache='none', #'in_memory',
                             phase_dir=args.phase_dir_val,
                             phase_cache='none') #'in_memory')
# psf_phase_dataset_val = PSF_Phase_Wrapper(dataset=place2_dataset_val, mask_type=args.mask_type, device='cpu')
dataloader_val = torch.utils.data.DataLoader(place2_dataset_val, 
                                             batch_size=args.batch_size, 
                                             shuffle=False, 
                                             pin_memory=True, 
                                             num_workers=8,
                                             persistent_workers=True)

optim = torch.optim.Adam(lr=args.lr, params=model.parameters(), amsgrad=False)

if args.loss_type == "grad":
    criterion = nn.MSELoss(reduction='mean')
elif args.loss_type == "l1":
    criterion = nn.L1Loss(reduction='mean')
elif args.loss_type == "l2":
    criterion = nn.MSELoss(reduction='mean')
elif args.loss_type == "mask_l2":
    mask = create_central_mask(image_size=256, mask_size=50) # use cutom function for loss
    mask = torch.tensor(mask, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(device)
else:
    raise ValueError("Loss type must be 'grad'")
model_dir = os.path.join('models_ker_est', args.name)
cond_mkdir(model_dir)
summaries_dir = os.path.join(model_dir, 'summaries')
cond_mkdir(summaries_dir)
checkpoints_dir = os.path.join(model_dir, 'checkpoints')
cond_mkdir(checkpoints_dir)

num_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
print (f'\n\nTraining model with {num_parameters} parameters\n\n')
checkpoint_path = os.path.join(checkpoints_dir, 'model_last.pth')
if resume:
    start_epoch = resume_training(model, optim, checkpoint_path, device=device)
else:
    start_epoch = 0
writer = SummaryWriter(summaries_dir)
total_steps = 0


train_losses = []
val_best = 1e10
for epoch in range(start_epoch, args.epochs):
    for step, batch in tqdm(enumerate(dataloader_train), desc=f"Epoch {epoch + 1}/{args.epochs}"):
        
        clean_img = batch['clean_img'].to(device)
        psfs = batch['psf'].to(device)
        B, C, H, W = psfs.size()
        # crop psf to 64x64
        psfs = central_crop(psfs, crop_size=args.patch_size, original_size=H)
        
        
        max_values = psfs.contiguous().view(psfs.shape[0], -1).max(dim=1, keepdim=True)[0]  # Find max along each index
        psfs = psfs / max_values.view(psfs.shape[0], 1, 1, 1) # Normalize the PSF from 0 to 1
        blurred_img = torch_convolve2d(clean_img, psfs, device)
        if args.gaussian_noise:
            blurred_img = add_gaussian_noise(blurred_img, std_range=(0.0, 0.01))
        patches = patchify_blurred_img(blurred_img, patch_size=args.patch_size, shuffle=args.patches_shuffle)

        # print(f'shape of psfs: {psfs.shape}')
        # print(f'min psf: {torch.min(psfs)}')
        # print(f'max psf: {torch.max(psfs)}')
        # print(f'shape of blurred_img: {blurred_img.shape}')
        # print(f'max blurred_img: {torch.max(blurred_img)}')
        # print(f'min blurred_img: {torch.min(blurred_img)}')
        
        # print(f'shape of patches: {patches.shape}')
        
        # raise ValueError("Stop here")
        
        pred_psf = model(patches)
        if args.loss_type == "grad":
            x_kernel = torch.tensor([[[[-1,0,1],[-2,0,2],[-1,0,1]]]]).to(device).type(torch.float32)
            y_kernel = torch.tensor([[[[-1,-2,-1],[0,0,0],[1,2,1]]]]).to(device).type(torch.float32)
            pred_grad_x = F.conv2d(pred_psf, x_kernel, padding=1)
            pred_grad_y = F.conv2d(pred_psf, y_kernel, padding=1)
            target_grad_x = F.conv2d(psfs, x_kernel, padding=1)
            target_grad_y = F.conv2d(psfs, y_kernel, padding=1)
            loss = criterion(pred_grad_x, target_grad_x) + criterion(pred_grad_y, target_grad_y)
        elif args.loss_type == "l1":
            loss = criterion(pred_psf, psfs)
        elif args.loss_type == "l2":
            loss = criterion(pred_psf, psfs)
        elif args.loss_type == "mask_l2":
            loss = masked_mse_loss(pred_psf, psfs, mask)
        else:
            raise ValueError("Loss type must be 'grad'")
        writer.add_scalar("train_loss",loss,total_steps)
        
        optim.zero_grad()
        loss.backward()
        optim.step()
        

        
        if not total_steps % 4000:#10000:#200:# 2350:
            torch.cuda.empty_cache()
            # tqdm.write("Epoch %d, outer loss %0.6f, iteration time %0.6f" % (epoch, loss, time.time() - start_time))
            log_psf_and_blurred_to_tensorboard(writer, pred_psf, psfs, blurred_img, total_steps, tag="train")
            

            torch.save({'epoch': epoch,
                        'model_state_dict': model.state_dict(),
                        'optimizer_state_dict':optim.state_dict(),
                        'loss':loss,
                        'total_steps':total_steps},
                        os.path.join(checkpoints_dir,f"model_{total_steps}.pth"))
            
            model.eval()
            with torch.no_grad():
                val_loss = []
                for step, batch in enumerate(dataloader_val):
                    clean_img = batch['clean_img'].to(device)
                    psfs = batch['psf'].to(device)
                    B, C, H, W = psfs.size()
                    # crop psf to 64x64
                    psfs = central_crop(psfs, crop_size=args.patch_size, original_size=H)
                    
                    max_values = psfs.contiguous().view(psfs.shape[0], -1).max(dim=1, keepdim=True)[0]  # Find max along each index
                    psfs = psfs / max_values.view(psfs.shape[0], 1, 1, 1) # Normalize the PSF from 0 to 1
                    blurred_img = torch_convolve2d(clean_img, psfs, device)
                    patches = patchify_blurred_img(blurred_img, patch_size=args.patch_size, shuffle=False)

                    pred_psf = model(patches)
                    if args.loss_type == "grad":
                        x_kernel = torch.tensor([[[[-1,0,1],[-2,0,2],[-1,0,1]]]]).to(device).type(torch.float32)
                        y_kernel = torch.tensor([[[[-1,-2,-1],[0,0,0],[1,2,1]]]]).to(device).type(torch.float32)
                        pred_grad_x = F.conv2d(pred_psf, x_kernel, padding=1)
                        pred_grad_y = F.conv2d(pred_psf, y_kernel, padding=1)
                        target_grad_x = F.conv2d(psfs, x_kernel, padding=1)
                        target_grad_y = F.conv2d(psfs, y_kernel, padding=1)
                        loss = criterion(pred_grad_x, target_grad_x) + criterion(pred_grad_y, target_grad_y)
                    elif args.loss_type == "l1":
                        loss = criterion(pred_psf, psfs)
                    elif args.loss_type == "l2":
                        loss = criterion(pred_psf, psfs)
                    elif args.loss_type == "mask_l2":
                        loss = masked_mse_loss(pred_psf, psfs, mask)
                    else:
                        raise ValueError("Loss type must be 'grad'")
                    val_loss.append(loss.detach().cpu().numpy())
                    log_psf_and_blurred_to_tensorboard(writer, pred_psf, psfs, blurred_img, total_steps, tag="val")

                writer.add_scalar("val_loss",np.mean(val_loss),total_steps)
                if val_best > np.mean(val_loss):
                    val_best = np.mean(val_loss)
                    torch.save({'epoch': epoch,
                                'model_state_dict': model.state_dict(),
                                'optimizer_state_dict':optim.state_dict(),
                                'loss':loss,
                                'total_steps':total_steps},
                                os.path.join(checkpoints_dir,"model_best.pth"))
                print("eval:{}".format(np.mean(val_loss)))
            if args.loss_type == "grad":
                del x_kernel, y_kernel, pred_grad_x, pred_grad_y, target_grad_x, target_grad_y
            del clean_img, blurred_img, psfs, pred_psf, loss
            torch.cuda.empty_cache()
            model.train()

        total_steps = total_steps+1

model.eval()
with torch.no_grad():
    val_loss = []
    for step, batch in enumerate(dataloader_val):
        clean_img = batch['clean_img'].to(device)
        psfs = batch['psf'].to(device)
        B, C, H, W = psfs.size()
        # crop psf to 64x64
        psfs = central_crop(psfs, crop_size=args.patch_size, original_size=H)
        
        max_values = psfs.contiguous().view(psfs.shape[0], -1).max(dim=1, keepdim=True)[0]  # Find max along each index
        psfs = psfs / max_values.view(psfs.shape[0], 1, 1, 1) # Normalize the PSF from 0 to 1
        blurred_img = torch_convolve2d(clean_img, psfs, device)
        patches = patchify_blurred_img(blurred_img, patch_size=args.patch_size, shuffle=False)
        pred_psf = model(patches)

        if args.loss_type == "grad":
            x_kernel = torch.tensor([[[[-1,0,1],[-2,0,2],[-1,0,1]]]]).to(device).type(torch.float32)
            y_kernel = torch.tensor([[[[-1,-2,-1],[0,0,0],[1,2,1]]]]).to(device).type(torch.float32)
            pred_grad_x = F.conv2d(pred_psf, x_kernel, padding=1)
            pred_grad_y = F.conv2d(pred_psf, y_kernel, padding=1)
            target_grad_x = F.conv2d(psfs, x_kernel, padding=1)
            target_grad_y = F.conv2d(psfs, y_kernel, padding=1)
            loss = criterion(pred_grad_x, target_grad_x) + criterion(pred_grad_y, target_grad_y)
        elif args.loss_type == "l1":
            loss = criterion(pred_psf, psfs)
        elif args.loss_type == "l2":
            loss = criterion(pred_psf, psfs)
        elif args.loss_type == "mask_l2":
            loss = masked_mse_loss(pred_psf, psfs, mask)
        else:
            raise ValueError("Loss type must be 'grad'")
        val_loss.append(loss.detach().cpu().numpy())
        log_psf_and_blurred_to_tensorboard(writer, pred_psf, psfs, blurred_img, total_steps, tag="val")
    writer.add_scalar("val_loss",np.mean(val_loss),total_steps)
    if val_best > np.mean(val_loss):
        val_best = np.mean(val_loss)
        torch.save({'epoch': epoch,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict':optim.state_dict(),
                    'loss':loss,
                    'total_steps':total_steps},
                    os.path.join(checkpoints_dir,"model_best.pth"))
    print("eval:{}".format(np.mean(val_loss)))
torch.save({'epoch': epoch,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict':optim.state_dict(),
            'loss':loss,
            'total_steps':total_steps},
            os.path.join(checkpoints_dir,"model_last.pth"))